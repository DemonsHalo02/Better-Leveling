...
const SYSTEM_QUOTES = [
  ...
  { text: "Weak hunters look for shortcuts. Strong hunters build the road.",        author: "The System" },
  { text: "Every level earned through struggle means more than one handed to you.", author: "The System" },
  { text: "The dungeon respects no excuses. Neither should you.",                   author: "The System" },
];

function renderDailyQuote() {
  const q = SYSTEM_QUOTES[Math.floor(Date.now()/86400000) % SYSTEM_QUOTES.length];
  return `
    <div style="
    ...
    ">
      ...
      <div style="font-size:14px;font-weight:600;color:var(--text);line-height:1.55;font-style:italic">"${q.text}"</div>
      <div style="font-family:var(--font-mono);font-size:9px;color:var(--text3);margin-top:6px">— ${q.author}</div>
    </div>
  `;
}

// ── HYDRATION QUICK-GLANCE (MIND tab TODAY) ───────────
function renderHydrationQuick() {
  const today    = new Date().toLocaleDateString();
  const data     = typeof getHydrationData    === 'function' ? getHydrationData()    : {};
  const cups     = data.date === today ? (data.cups||0) : 0;
  const cupGoal  = typeof getWaterCupGoal     === 'function' ? getWaterCupGoal()     : 8;
  const cupSzMl  = typeof getCupSizeMl        === 'function' ? getCupSizeMl()        : 250;
  const goalMl   = typeof getWaterGoalMl      === 'function' ? getWaterGoalMl()      : 2000;
  const totalMl  = cups * cupSzMl;
  const totDisp  = typeof mlToDisplay         === 'function' ? mlToDisplay(totalMl)  : totalMl+'ml';
  const goalDisp = typeof getWaterGoalDisplay === 'function' ? getWaterGoalDisplay() : '2000ml';
  const pct      = Math.min(100, Math.round((totalMl/goalMl)*100));
  const goalHit  = totalMl >= goalMl;
  return `
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
      <svg viewBox="0 0 56 56" style="width:46px;height:46px;flex-shrink:0">
        ...
        <text x="28" y="33" text-anchor="middle" font-family="Orbitron" font-size="12" font-weight="700" fill="#00b4ff">${cups}</text>
      </svg>
      <div style="flex:1">
        <div style="font-family:var(--font-hud);font-size:16px;color:var(--accent)">${totDisp} <span style="font-size:11px;color:var(--text3)">/ ${goalDisp}</span></div>
        <div style="font-family:var(--font-mono);font-size:9px;color:${goalHit?'var(--green)':'var(--text3)'};margin-top:2px">${goalHit?'✓ Daily goal reached!':`${cupGoal-cups} cups to go`}</div>
      </div>
    </div>
    <div style="display:flex;gap:4px;margin-bottom:8px">
      ${Array.from({length:Math.min(10,cupGoal)},(_,n)=>`<div style="flex:1;height:16px;background:${n<cups?'rgba(0,180,255,0.35)':'rgba(0,180,255,0.05)'};border:1px solid ${n<cups?'var(--accent)':'var(--border)'};border-radius:3px"></div>`).join('')}
    </div>
    <div style="display:flex;gap:8px">
      <button class="btn-primary" style="flex:1" onclick="addWaterCup();renderSelfImprovePage()">+ Add Cup</button>
      <button class="btn-secondary" onclick="removeWaterCup();renderSelfImprovePage()">−</button>
    </div>
    <div style="font-family:var(--font-mono);font-size:9px;color:var(--text3);margin-top:6px;text-align:center">Manual entry & full log → FOOD tab</div>
  `;
}
...
function renderTabToday(el) {
  ...
  // Hydration quick-glance only — full tracker is on FOOD tab
  el.innerHTML += collapsible('hydration-quick', 'Hydration', '💧', renderHydrationQuick(), true);
...
function renderTabBody(el) {
  ...
  // Calorie/macro calculator lives on the FOOD page to avoid duplicates
  el.innerHTML += `
    <div style="background:rgba(0,180,255,0.04);border:1px solid rgba(0,180,255,0.15);border-radius:8px;padding:12px 14px;margin-bottom:10px;display:flex;align-items:center;gap:12px">
      <div style="font-size:22px">⚖️</div>
      <div style="flex:1">
        <div style="font-size:13px;font-weight:600;color:var(--text)">Calorie & Macro Calculator</div>
        <div style="font-family:var(--font-mono);font-size:9px;color:var(--text3);margin-top:2px">Available on the FOOD tab — scroll to the bottom</div>
      </div>
    </div>
  `;
}
...
// ── HYDRATION ─────────────────────────────────────────
// getHydrationData / saveHydrationData defined in foodscanner.js
// addWaterCup / removeWaterCup also defined in foodscanner.js
// We just render here — no duplicate XP logic
function renderHydrationContent() {
  const today    = new Date().toLocaleDateString();
  const data     = typeof getHydrationData    === 'function' ? getHydrationData()    : {};
  const cups     = data.date === today ? (data.cups||0) : 0;
  const cupGoal  = typeof getWaterCupGoal     === 'function' ? getWaterCupGoal()     : 8;
  const cupDisp  = typeof getCupSizeDisplay   === 'function' ? getCupSizeDisplay()   : '250ml';
  const goalDisp = typeof getWaterGoalDisplay === 'function' ? getWaterGoalDisplay() : '2000ml';
  const cupSzMl  = typeof getCupSizeMl        === 'function' ? getCupSizeMl()        : 250;
  const goalMl   = typeof getWaterGoalMl      === 'function' ? getWaterGoalMl()      : 2000;
  const totalMl  = cups * cupSzMl;
  const totDisp  = typeof mlToDisplay         === 'function' ? mlToDisplay(totalMl)  : totalMl+'ml';
  const unit     = typeof getVolumeUnit       === 'function' ? getVolumeUnit()       : 'ml';
  const isImp    = typeof getIsImperial       === 'function' ? getIsImperial()       : false;
  const pct      = Math.min(100, Math.round((totalMl / goalMl) * 100));
  const goalHit  = totalMl >= goalMl;
    ...
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:12px">
      ...
      <div style="flex:1">
        <div style="font-family:var(--font-hud);font-size:18px;color:var(--accent)">
          ${totDisp} <span style="font-size:11px;color:var(--text3)">/ ${goalDisp}</span>
        </div>
        <div style="font-family:var(--font-mono);font-size:9px;color:${goalHit?'var(--green)':'var(--text3)'};margin-top:2px">
          ${goalHit ? '✓ Daily goal reached!' : `${cupGoal-cups} cups to go`}
        </div>
      </div>
    </div>
    <div style="display:flex;gap:4px;margin-bottom:10px">
      ${Array.from({length:Math.max(8,cupGoal)},(_,n)=>`
        <div style="flex:1;height:22px;background:${n<cups?'rgba(0,180,255,0.35)':'rgba(0,180,255,0.05)'};border:1px solid ${n<cups?'var(--accent)':'var(--border)'};border-radius:4px"></div>
      `).join('')}
    </div>
    <div style="display:flex;gap:6px;margin-bottom:8px">
      <button class="btn-primary" style="flex:1" onclick="addWaterCup();renderSelfImprovePage()">+ ${cupDisp}</button>
      <button class="btn-secondary" onclick="removeWaterCup();renderSelfImprovePage()">−</button>
    </div>
    <div style="display:flex;gap:8px">
      <input type="number" id="water-manual-mind" class="sys-input"
        placeholder="Enter ${unit} amount..." inputmode="decimal" style="flex:1;font-size:13px"/>
      <button class="btn-secondary" onclick="addWaterManualMind()">ADD</button>
    </div>
    <div style="font-family:var(--font-mono);font-size:9px;color:var(--text3);margin-top:5px">
      e.g. ${isImp?'16':'500'} for a large bottle
    </div>
  `;
}

function addWaterManualMind() {
  const input = document.getElementById('water-manual-mind');
  const val   = parseFloat(input?.value);
  if (!val || val <= 0) { showNotif('[ ERROR ] Enter a valid amount'); return; }
  const isImp = typeof getIsImperial  === 'function' && getIsImperial();
  const ml    = isImp ? val * 29.5735 : val;
  const cupSz = typeof getCupSizeMl   === 'function' ? getCupSizeMl()   : 250;
  const cups  = Math.max(1, Math.round(ml / cupSz));
  const today = new Date().toLocaleDateString();
  const data  = typeof getHydrationData === 'function' ? getHydrationData() : {};
  const prev  = data.date === today ? (data.cups||0) : 0;
  const next  = Math.min(30, prev + cups);
  const goal  = typeof getWaterCupGoal === 'function' ? getWaterCupGoal() : 8;
  if (typeof saveHydrationData === 'function') saveHydrationData(next);
  // XP only on first goal hit
  if (next >= goal && prev < goal) {
    addXP(20, 'vit');
    showNotif(`[ HYDRATION ] Goal reached! +20 XP`, 'gold');
  } else {
    const disp = typeof mlToDisplay === 'function' ? mlToDisplay(ml) : ml+'ml';
    showNotif(`[ HYDRATION ] +${disp}`);
  }
  if (input) input.value = '';
  renderSelfImprovePage();
}

// Remove duplicate storage functions — defined in foodscanner.js
// getHydrationData / saveHydrationData are NOT redefined here
...
function renderMeasurementsContent() {
  ...
  const isImp  = s.units === 'imperial';

  // Weight display with units
  function dispW(kg) {
    if (!kg) return null;
    return isImp ? `${(kg*2.20462).toFixed(1)} lbs` : `${kg} kg`;
  }
  function dispH(cm) {
    if (!cm) return null;
    if (isImp) { const i=Math.round(cm/2.54); return `${Math.floor(i/12)}'${i%12}"`; }
    return `${cm} cm`;
  }
  ...
  let chart = '';
  if (wData.length >= 2) {
    const vals = wData.map(l => isImp ? l.weight*2.20462 : l.weight);
    const mn=Math.min(...vals)-1, mx=Math.max(...vals)+1;
    const W=280,H=80,pad=8;
    const pts=wData.map((d,i)=>({x:pad+(i/(wData.length-1))*(W-pad*2),y:H-pad-((( isImp?d.weight*2.20462:d.weight)-mn)/(mx-mn))*(H-pad*2),...d}));
    const pathD=pts.map((p,i)=>`${i===0?'M':'L'}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' ');
    const areaD=pathD+` L${pts[pts.length-1].x},${H-pad} L${pts[0].x},${H-pad} Z`;
    const rawTrend=(isImp?(wData[wData.length-1].weight*2.20462):(wData[wData.length-1].weight))-(isImp?(wData[0].weight*2.20462):(wData[0].weight));
    const trend=rawTrend.toFixed(1);
    const unit = isImp ? 'lbs' : 'kg';
    chart=`<div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;margin-bottom:4px"><div style="font-family:var(--font-mono);font-size:9px;color:var(--text3)">WEIGHT TREND (${unit})</div><div style="font-family:var(--font-mono);font-size:10px;color:${parseFloat(trend)<=0?'var(--green)':'var(--red)'}">${parseFloat(trend)>0?'+':''}${trend}${unit}</div></div><svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto"><defs><linearGradient id="wg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#00b4ff" stop-opacity="0.25"/><stop offset="100%" stop-color="#00b4ff" stop-opacity="0.02"/></linearGradient></defs><path d="${areaD}" fill="url(#wg)"/><path d="${pathD}" fill="none" stroke="#00b4ff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>${pts.map(p=>`<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3" fill="#00b4ff"/>`).join('')}</svg></div>`;
  }

  return `${chart}${latest ? `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:12px">
      ${latest.weight ? `<div style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:8px;text-align:center"><div style="font-family:var(--font-hud);font-size:16px;color:var(--accent)">${dispW(latest.weight)}</div><div style="font-family:var(--font-mono);font-size:9px;color:var(--text3)">WEIGHT${prev?.weight?`<br><span style="color:${(latest.weight-prev.weight)<=0?'var(--green)':'var(--red)'}">${latest.weight-prev.weight>0?'+':''}${(latest.weight-prev.weight).toFixed(1)} kg</span>`:''}</div></div>` : ''}
      ${latest.chest ? `<div style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:8px;text-align:center"><div style="font-family:var(--font-hud);font-size:16px;color:var(--accent)">${latest.chest}${isImp?'"':'cm'}</div><div style="font-family:var(--font-mono);font-size:9px;color:var(--text3)">CHEST</div></div>` : ''}
      ${latest.waist ? `<div style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:8px;text-align:center"><div style="font-family:var(--font-hud);font-size:16px;color:var(--accent)">${latest.waist}${isImp?'"':'cm'}</div><div style="font-family:var(--font-mono);font-size:9px;color:var(--text3)">WAIST</div></div>` : ''}
      ${latest.arms ? `<div style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:8px;text-align:center"><div style="font-family:var(--font-hud);font-size:16px;color:var(--accent)">${latest.arms}${isImp?'"':'cm'}</div><div style="font-family:var(--font-mono);font-size:9px;color:var(--text3)">ARMS</div></div>` : ''}
    </div>
  ` : ''}
  <button onclick="openMeasurementsSheet()" style="width:100%;padding:10px;background:transparent;border:1px dashed var(--border2);border-radius:6px;color:var(--text3);font-family:var(--font-ui);font-size:12px;cursor:pointer">+ Log Measurements</button>
  `;
}
...
function openMeasurementsSheet() {
  const s    = typeof getSettings === 'function' ? getSettings() : {};
  const isImp = s.units === 'imperial';
  const wLabel = isImp ? 'Weight (lbs)' : 'Weight (kg)';
  const mLabel = isImp ? 'inches (")' : 'cm';

  showBottomSheet('Log Measurements', `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">
      <div>
        <label style="font-family:var(--font-mono);font-size:9px;color:var(--text3);display:block;margin-bottom:4px">${wLabel}</label>
        <input type="number" class="sys-input" id="bm-weight" placeholder="${isImp?'185':'84'}" inputmode="decimal" step="0.1"/>
      </div>
      <div>
        <label style="font-family:var(--font-mono);font-size:9px;color:var(--text3);display:block;margin-bottom:4px">CHEST (${mLabel})</label>
        <input type="number" class="sys-input" id="bm-chest" inputmode="numeric"/>
      </div>
      <div>
        <label style="font-family:var(--font-mono);font-size:9px;color:var(--text3);display:block;margin-bottom:4px">WAIST (${mLabel})</label>
        <input type="number" class="sys-input" id="bm-waist" inputmode="numeric"/>
      </div>
      <div>
        <label style="font-family:var(--font-mono);font-size:9px;color:var(--text3);display:block;margin-bottom:4px">ARMS (${mLabel})</label>
        <input type="number" class="sys-input" id="bm-arms" inputmode="numeric"/>
      </div>
    </div>
    <button class="btn-primary" onclick="
      const raw=parseFloat(document.getElementById('bm-weight')?.value)||null;
      const imp=${isImp};
      // Store weight always in kg
      const w=raw?(imp?parseFloat((raw/2.20462).toFixed(2)):raw):null;
      const c=parseFloat(document.getElementById('bm-chest')?.value)||null;
      const wa=parseFloat(document.getElementById('bm-waist')?.value)||null;
      const a=parseFloat(document.getElementById('bm-arms')?.value)||null;
      if(!w&&!c&&!wa&&!a){showNotif('Enter at least one value');return;}
      const logs=getBodyLogs();
      logs.push({date:new Date().toLocaleDateString(),weight:w,chest:c,waist:wa,arms:a});
      localStorage.setItem('sys_body_hunter',JSON.stringify(logs));
      addXP(15,'sense');
      closeBottomSheet();
      showNotif('[ SAVED ] Measurements logged +15 XP');
    "><span>SAVE</span><div class='btn-arrow'>▶</div></button>
  `, () => renderSelfImprovePage());
}
