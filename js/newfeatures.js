...
function renderTabToday(el) {
  ...
  el.innerHTML += collapsible('hydration', 'Hydration',       '💧', renderHydrationContent(),  true);
...
function renderHydrationContent() {
  const today=new Date().toLocaleDateString(), data=getHydrationData(), cups=data.date===today?(data.cups||0):0, goal=8, pct=Math.min(100,Math.round((cups/goal)*100));
  return `<div style="display:flex;align-items:center;gap:14px;margin-bottom:12px"><svg viewBox="0 0 56 56" style="width:56px;height:56px;flex-shrink:0"><circle cx="28" cy="28" r="24" fill="none" stroke="rgba(0,180,255,0.1)" stroke-width="5"/><circle cx="28" cy="28" r="24" fill="none" stroke="#00b4ff" stroke-width="5" stroke-dasharray="${2*Math.PI*24}" stroke-dashoffset="${2*Math.PI*24*(1-pct/100)}" stroke-linecap="round" transform="rotate(-90 28 28)" style="transition:stroke-dashoffset 0.4s"/><text x="28" y="33" text-anchor="middle" font-family="Orbitron" font-size="12" font-weight="700" fill="#00b4ff">${cups}</text></svg><div style="flex:1"><div style="font-family:var(--font-hud);font-size:18px;color:var(--accent)">${cups*250}ml <span style="font-size:11px;color:var(--text3)">/ 2000ml</span></div><div style="font-family:var(--font-mono);font-size:9px;color:var(--text3);margin-top:2px">${goal-cups>0?`${goal-cups} cups to go`:'✓ Goal reached!'}</div></div></div><div style="display:flex;gap:5px;margin-bottom:12px">${[1,2,3,4,5,6,7,8].map(n=>`<div style="flex:1;height:22px;background:${n<=cups?'rgba(0,180,255,0.35)':'rgba(0,180,255,0.05)'};border:1px solid ${n<=cups?'var(--accent)':'var(--border)'};border-radius:4px"></div>`).join('')}</div><div style="display:flex;gap:8px"><button class="btn-primary" style="flex:1" onclick="addCup()"><span>+ 250ml</span></button><button class="btn-secondary" onclick="removeCup()">−</button></div>`;
}
function addCup() { const t=new Date().toLocaleDateString(),d=getHydrationData(),cups=d.date===t?Math.min(20,(d.cups||0)+1):1; saveHydrationData(cups); if(cups===8){addXP(20,'vit');showNotif('[ HYDRATION ] Goal reached! +20 XP','gold');}else showNotif(`[ HYDRATION ] ${cups}/8 cups`); renderSelfImprovePage(); }
function removeCup() { const t=new Date().toLocaleDateString(),d=getHydrationData(),cups=d.date===t?Math.max(0,(d.cups||0)-1):0; saveHydrationData(cups); renderSelfImprovePage(); }
function getHydrationData() { try{return JSON.parse(localStorage.getItem('sys_hydration_hunter')||'{}');}catch{return{};} }
function saveHydrationData(cups) { const d=getHydrationData(); d.date=new Date().toLocaleDateString(); d.cups=cups; localStorage.setItem('sys_hydration_hunter',JSON.stringify(d)); }
